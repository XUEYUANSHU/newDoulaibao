<template>
    <div>
        <commonHeader url="-1" title="银行卡"/>
        <div class="binkListWrapper" v-if="!showDelete">
            <div class="bankList" v-for="(item,idx) in bankList" :key=idx @click="showTips(true, idx)">
                <div><i class="iconfont icon-bink">&#xe617;</i><span
                    class="bandName">{{item.bankName}}</span><span>{{item.bankNum}}</span></div>
            </div>
        ...
        <div class="cancelBank" v-if="showDelete">
            <div class="bankList">
                <div><span>您将解绑</span><span
                    class="bandName">{{bankList[idx].bankName}}</span><span>{{bankList[idx].bankNum}}</span></div>
            </div>
